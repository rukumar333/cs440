...
#define Deque_DEFINE(t)							\
  ...
  void resize(Deque_##t *deq){						\
    ...
    Deque_##t##_Element *array_new = new Deque_##t##_Element[deq->capacity * 2]; \
    if (deq->begin_index < deq->end_index)				\
      std::memcpy(array_new, deq->array + deq->begin_index, (deq->end_index - deq->begin_index) * sizeof(Deque_##t##_Element)); \
    else {								\
      std::memcpy(array_new, deq->array + deq->begin_index, (deq->capacity - deq->begin_index) * sizeof(Deque_##t##_Element)); \
      std::memcpy(array_new + (deq->capacity - deq->begin_index), deq->array, deq->end_index * sizeof(Deque_##t##_Element)); \
    }									\
    deq->begin_index = 0;						\
  ...
  void pop_back(Deque_##t *deq) {					\
    ...
    if (deq->end_index == 0)						\
      ...
      deq->end_index = deq->end_index - 1;				\
  									...
  									\
  t &front(Deque_##t *deq) {						\
    return (deq->array + deq->begin_index)->item;			\
  }									\
  									\
  t &back(Deque_##t *deq) {						\
    return (deq->array + deq->end_index - 1)->item;			\
  }									\
  									\
  size_t size(Deque_##t *deq) {						\
